...
/// <summary>
/// Functional test to verify that the Nuclia SDK can download a field file by URL.
/// This test ensures that the SDK can retrieve binary file content using a complete resource URL.
/// Expected result: The API call succeeds and returns binary data.
/// </summary>
public class DownloadFieldFileByUrlFunctionalTest : WriteOperationFunctionalTestBase
{
    /// <summary>
    /// Verifies that the SDK can download a field file by URL.
    /// The test passes if the API call succeeds and returns binary content.
    /// </summary>
    [Fact(DisplayName = "Can download field file by URL using DownloadFieldFileAsync")]
    [Trait("Category", "Functional")]
    [Trait("Service", "ResourceService")]
    public async Task CanDownloadFieldFileByUrl()
    {
        // Step 1: Create a resource with a file field
        var payload = new CreateResourcePayload
        {
            Title = $"{TestPrefix}-download-url-test",
            Summary = "Test resource for file download by URL",
        ...
        // Step 2: Construct the resource URL path and download the file
        var resourceUrl = $"/kb/{KnowledgeBoxId}/resource/{resourceId}/file/testfile/download/field";
        var downloadResponse = await Client.Resources.DownloadFieldFileAsync(resourceUrl);
